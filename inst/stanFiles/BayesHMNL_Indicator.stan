...
// data {
  ...
  data {
    ...
    matrix[K,K]  RM ; // residual maker matrix to force sum to 1 constraints on theta and Beta
  ...
  transformed parameters {
    ...
    // for (k in 1:(K-1)) {
      ...
      // }
      // Impose sum to zero constraints on raw pop and indivicual coeffs
      Theta = RM * Theta_raw;
      for (iSj in 1:nSj){
  ...
  model {
    ...
    Theta_raw ~ normal(0, 2); // changed from norm(0,10)
    ...
    //likelihood
    Beta_raw ~ multi_normal_cholesky(Theta, L_Sigma);
    for (t in 1:nT) { // was for (r in 1:nSj) {
