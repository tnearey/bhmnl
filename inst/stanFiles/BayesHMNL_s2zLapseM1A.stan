// BayesMNLs2zM1A.stan
// Mark 1A-- made beta deviation from theta
// Like M1wth transformed variabls block moved to temporaries Model block
//
...
// data {
  ...
  parameters {
    // vector[D] beta[J];
    vector<lower= log(1.0/N), upper=0> [C] u [J]; // lapse rate and proportion parameters
    vector[D]  beta[J];
  ...
  // eliminated transformed parameters block except for L_sigma
  ...
  model {
    ...
    vector[C] exp_u [J];
    real sum_exp_u[J];
    vector[C] lambda_s[J]; // s si sneeze factor
    vector[C] s[J];
    real lambda[J] ;
    ...
    // for (k in 1:D) {
      ...
      // for (k in 1:(D-1)) {
        ...
        // for (iSj in 1:J){
          ...
          for (j in 1:J){
            exp_u[j]=exp(u[j]);
            sum_exp_u[j]=sum(exp(u[j]));
            lambda_s[j] = exp_u[j]/(1.0+sum_exp_u[j]);
            lambda[j]=sum(lambda_s[j]);
            s[j]= exp_u[j]/sum_exp_u[j];
          }
          ...
          for (t in 1:N) { // was for (r in 1:J) {
            ...
            q = softmax(XB[t]);
            p= (1-lambda[jj[t]])*q + lambda[jj[t]]*s[jj[t]];
            // if (t==1)
            //   print("t ", t, "Y[t] ", Y[t])
            Y[t] ~ categorical(p);
          }
